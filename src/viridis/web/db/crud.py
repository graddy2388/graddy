"""
viridis.web.db.crud – CRUD helpers for the web GUI database.
...
def upsert_host_software(
    ...
    cve_ids = json.dumps([c["id"] for c in (cves or [])])
    cve_summary = json.dumps((cves or [])[:10])
    db.execute(
        """
        INSERT INTO host_software (host_ip, name, version, source, port, cve_ids, cve_summary, last_seen)
        VALUES (?, ?, ?, ?, ?, ?, ?, datetime('now'))
        ON CONFLICT(host_ip, name, source, port) DO UPDATE SET
            version = excluded.version,
            cve_ids = excluded.cve_ids,
            cve_summary = excluded.cve_summary,
            last_seen = datetime('now')
        """,
        (host_ip, name, version, source, port, cve_ids, cve_summary),
    )
    db.commit()


def get_host_software(db, host_ip: str) -> List[Dict[str, Any]]:
    cur = db.execute(
        "SELECT * FROM host_software WHERE host_ip = ? ORDER BY name",
        (host_ip,),
    )
    rows = _rows(cur.fetchall())
    for row in rows:
        try:
            row["cve_ids"] = json.loads(row.get("cve_ids") or "[]")
        except Exception:
            row["cve_ids"] = []
        try:
            row["cve_summary"] = json.loads(row.get("cve_summary") or "[]")
        except Exception:
            row["cve_summary"] = []
    return rows
